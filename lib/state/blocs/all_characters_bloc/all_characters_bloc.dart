...
class AllCharactersBloc extends Bloc<AllCharacterEvent, AllCharacterState> {
  ...
  AllCharactersBloc(this._characterRepository, this._filterBloc)
    ...
    _filterBloc.stream.listen((state) {
      if (state is FilterdCharactersState) {
        add(ResetSearchPage()); //reset more stuff
        add(FilterCharactersEvent(params: state.params!));
      } else if (state is UnFilterdCharactersState) {}
    });
    allCharactersScrollController.addListener(() {
      lastScrollPosition = allCharactersScrollController.position;
      if (_filterBloc.state is FilterdCharactersState) {
        add(LoadMoreFilterdCharactersEvent(params: _filterBloc.state.params!));
      } else {
        add(LoadMoreCharactersEvent());
      }
    });

    on<FilterCharactersEvent>((event, emit) async {
